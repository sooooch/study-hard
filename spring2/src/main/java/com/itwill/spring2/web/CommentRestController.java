...
public class CommentRestController {
	...
	public ResponseEntity<Integer> registerComment(@RequestBody CommentCreateDto dto) {
		// @ReauestBody: Ajax 요청의 요청 패킷 몸동(body)에 포함된 데이터를 읽어서 자바 객체로 변환.

		int rs = commentService.create(dto);
		return ResponseEntity.ok(rs);
	}

	@PutMapping("/{cid}")
	public ResponseEntity<Integer> update(@PathVariable int cid,@RequestBody CommentUpdateDto dto) {
		dto.setId(cid);
		int rs = commentService.update(dto);
		return ResponseEntity.ok(rs);
	}

	@DeleteMapping("/{cid}")
	public ResponseEntity<Integer> delete(@PathVariable int cid) {
		
		int rs = commentService.deleteByID(cid);
		return ResponseEntity.ok(rs);
